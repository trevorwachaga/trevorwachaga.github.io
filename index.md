---
layout: default
title: Home
---

# Welcome to My Portfolio

Hi, I'm Trevor Wachaga.

I'm an Ethical Hacking & Cyber Security Graduate.

Here, you’ll find details about my **background, projects, and ways to contact me**.<br>

<div style="display: flex; align-items: center; gap: 40px; max-width: 900px; margin: auto;">       
    <div>
      <div class="projects-grid">
          <a href="/pages/about.md" target="_blank" class="project-card">
              <img src="{{ '/images/linkedin.jpg' | relative_url }}" alt="About me">
              <h3>About me</h3>
          </a>
          <a href="/pages/projects.md" target="_blank" class="project-card">
              <img src="{{ '/images/projects.png' | relative_url }}" alt="Projects">
              <h3>Projects</h3>
          </a>
          <a href="/pages/contact.md" target="_blank" class="project-card">
              <img src="{{ '/images/contact.png' | relative_url }}" alt="Contact">
              <h3>Contact me</h3>
          </a>
      <div>
    <div>
<div>